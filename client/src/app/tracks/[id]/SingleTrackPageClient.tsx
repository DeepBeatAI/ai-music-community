...
const DeleteConfirmationModal = lazy(() =>
  import("@/components/library/DeleteConfirmationModal").then((mod) => ({
...
export default function SingleTrackPageClient() {
   ...
   * and system events. They automatically dismiss after 5 seconds.
   *
   * @param message - The message to display in the toast
   * @param type - The type of toast (success, error, or info) which determines styling
   *
   * @example
   * ```typescript
  ...
  const showToast = (message: string, type: ToastType = "info") => {
    ...
    // Auto-dismiss after 5 seconds to avoid cluttering the UI
    setTimeout(() => {
  ...
  /**
   * Dismisses a toast notification immediately
   *
   * This function is called when the user clicks the dismiss button on a toast.
   *
  ...
  /**
   * Main effect to fetch all track data and related information
   *
   * This effect runs when the component mounts or when dependencies change.
   * It fetches:
   * - Track metadata (title, author, description, etc.)
   * - User profile information for the track uploader
   * - Playlist memberships
   * - Like count from associated posts
   *
   * The effect waits for authentication to complete before fetching to ensure
   * proper permission checks for private tracks.
   */
  useEffect(() => {
    /**
     * Fetches comprehensive track data with all related information
     *
     * This function performs multiple database queries to gather all data
     * needed to display the track page. It uses optimistic error handling
     * to gracefully handle non-existent tracks without console errors.
     *
     * @returns Promise resolving to extended track data or null if not found
     * @throws Error if track is private and user lacks permission
     */
    const fetchTrackData =
      async (): Promise<ExtendedTrackWithMembership | null> => {
        if (!trackId) {
        ...
        try {
          ...
          // This prevents console errors for expected 404 cases (deleted tracks, invalid IDs)
          ...
          if (
            !trackData.is_public &&
            (!user || user.id !== trackData.user_id)
          ) {
          ...
          // This ensures consistent like count calculation across the application
          ...
          console.log(
            "[Like Count Debug] Posts data for track:",
            trackId,
            postsData,
          );
          ...
          if (postsData && postsData.length > 0) {
            ...
            // This post ID is needed for the LikeButton component to function correctly
            postId = postsData[0].id;

            // Sum up likes from all posts (in case there are multiple posts for the same track)
            // This handles edge cases where a track might be shared in multiple posts
            ...
            console.log(
              "[Like Count Debug] Calculated like count from posts:",
              likeCountValue,
            );
          } else {
            // If no post exists, fall back to the like_count column in tracks table
            // This ensures we always display a like count even for tracks without posts
            likeCountValue = trackData.like_count || 0;
            console.log(
              "[Like Count Debug] No posts found, using track.like_count:",
              likeCountValue,
            );
          }
          ...
          if (
            trackData.playlist_tracks &&
            Array.isArray(trackData.playlist_tracks)
          ) {
            trackData.playlist_tracks.forEach(
              (pt: { playlist: { id: string; name: string } | null }) => {
                if (pt.playlist) {
    ...
    const loadTrackData = async () => {
      ...
      try {
        ...
        performanceMetrics.current.metadataLoadTime =
          performance.now() - performanceMetrics.current.pageLoadStart;
        console.log(
          "[Performance] Metadata loaded in",
          performanceMetrics.current.metadataLoadTime.toFixed(2),
          "ms",
        );
   ...
   * the user explicitly requests it (by clicking the play button). This
   * improves initial page load performance.
   *
   * Features:
   * - Deferred audio loading (waits for user interaction)
   * - Automatic retry with exponential backoff (up to 3 attempts)
   * - Performance tracking (audio ready time)
   * - Error handling with user-friendly messages
   */
  useEffect(() => {
    let retryCount = 0;
    const maxRetries = 3;
    const retryDelay = 1000; // 1 second base delay (increases exponentially)

    /**
     * Loads and caches the audio URL
     *
     * Uses getCachedAudioUrl utility to optimize audio loading through caching.
    ...
    const loadAudioUrl = async () => {
      if (shouldLoadAudio && track?.file_url && !cachedAudioUrl) {
        try {
          console.log(
            "[Audio Cache] Attempting to load audio URL, retry:",
            retryCount,
            {
              trackId: track.id,
              userId: user?.id,
          ...
          console.log(
            "[Audio Cache] Successfully loaded audio URL in",
            loadTime.toFixed(2),
            "ms",
            {
              trackId: track.id,
          ...
          performanceMetrics.current.audioReadyTime =
            performance.now() - performanceMetrics.current.pageLoadStart;
          console.log(
            "[Performance] Audio ready in",
            performanceMetrics.current.audioReadyTime.toFixed(2),
            "ms",
          );
          ...
          const err =
            error instanceof Error
              ? error
              : new Error("Unknown audio load error");
          ...
          if (retryCount < maxRetries) {
            retryCount++;
            console.log(
              "[Audio Cache] Retrying audio load, attempt",
              retryCount,
              "of",
              maxRetries,
              {
            ...
            const errorMessage =
              "Failed to load audio file. Please check your internet connection and try again.";
  ...
  /**
   * Triggers audio loading on user interaction
   *
   * This function is called when the user clicks the "Load audio player" button.
   * It sets the shouldLoadAudio flag which triggers the audio loading effect.
   *
   * Performance tracking: Measures the time from user interaction to state update
   * to ensure we meet the < 100ms interaction response target.
   */
  ...
  /**
   * Handles track deletion confirmation
   *
   * This function is called when the user confirms track deletion in the modal.
   * It deletes the track from the database and navigates back to the previous page.
   *
   * Flow:
   * 1. Call deleteTrack utility function
   * 2. Show success/error toast notification
   * 3. Navigate back after 500ms delay (allows user to see success message)
   *
   * @remarks
   * Only track owners can delete tracks. The delete button is only shown
   * to authenticated users who own the track.
   */
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (success) {
        showToast("Track deleted successfully", "success");
        // Navigate back after successful deletion (delay allows user to see success message)
        setTimeout(() => {
          handleBack();
        }, 500);
      } else {
  ...
  /**
   * Handles back navigation with smart fallback logic
   *
   * This function implements intelligent back navigation that considers
   * the user's navigation history and authentication state.
   *
   * Navigation logic:
   * 1. If browser history exists: Use router.back() to return to previous page
   * 2. If no history and user is authenticated: Navigate to dashboard
   * 3. If no history and user is not authenticated: Navigate to home page
   *
   * This ensures users always have a way to navigate away from the track page,
   * even if they accessed it directly via a shared URL.
   *
   * @remarks
   * Performance tracking: Measures interaction response time to ensure
   * we meet the < 100ms target for user interactions.
   */
  const handleBack = () => {
    const interactionStart = performance.now();

    if (window.history.length > 1) {
      // User has navigation history - go back to previous page
      router.back();
    } else {
      // No navigation history - use fallback based on authentication state
      if (user) {
    ...
    performanceMetrics.current.interactionTimes.push(responseTime);
    console.log(
      "[Performance] Back button response time:",
      responseTime.toFixed(2),
      "ms",
    );
  };
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      console.log(
        "Single track page unmounting - WavesurferPlayer will handle cleanup",
      );
      ...
      console.log("[Performance] Session Summary:", {
        ...
        interactionTimes: metrics.interactionTimes.map(
          (t) => `${t.toFixed(2)}ms`,
        ),
      });
    };
          ...
          <div className="mb-4 bg-orange-900/30 border border-orange-500/50 rounded-lg p-3 flex items-center space-x-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-orange-400/80 text-xs">
                Some features may be unavailable. Actions will be retried when
          ...
          <div className="max-w-2xl mx-auto">
            ...
            {errorType === "404" && (
              <div className="bg-gray-800 border border-gray-700 rounded-lg p-8 text-center">
                ...
                <p className="text-gray-400 mb-6">
                  The track you&apos;re looking for doesn&apos;t exist or has
                  been deleted.
                </p>
            ...
            {errorType === "403" && (
              <div className="bg-gray-800 border border-gray-700 rounded-lg p-8 text-center">
                ...
                <p className="text-gray-400 mb-6">
                  This track is private. You don&apos;t have permission to view
                  it.
                </p>
                <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4">
                  <button
                    ...
                    >
                      ...
                      >
                        <FollowButton
                          ...
                          username={
                            track.user?.username || track.author || "User"
                          }
                          size="md"
                          variant="primary"
                        />
                  ...
                  fallback={
                    <div className="bg-red-900/20 border border-red-500/30 rounded-lg p-4">
                      ...
                      <p className="text-gray-300 text-sm mb-4">
                        The audio player encountered an error. Please try
                        refreshing the page.
                      </p>
                      <button
